...
#set ($aliyunProfile = $instance.getAttribute("aliyun_profile_name"))
set_aliyun_profile --profile_name "$aliyunProfile";

mkdir -p "${D}TERRAFORM_TEMP_DIR";
cp -R . "${D}TERRAFORM_TEMP_DIR";
#foreach ($terraformInstance in $instance.getInstancesByFileExtensions(true, ".tf", ".tpl", ".tfvars"))
